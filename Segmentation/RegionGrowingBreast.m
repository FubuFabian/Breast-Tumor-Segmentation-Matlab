function [P, J] = RegionGrowingBreast(cIM, initPos, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify)
...
% initial pixel value
seedRegion = cIM(initPos(1)-R:initPos(1)+R,initPos(2)-R:initPos(2)+R);
    ...
    for j = -R:R
      for k = -R:R
        ...
        if xv+i > 0  &&  xv+i <= nRow &&...          % within the x-bounds?
           ...
           cIM(xv+i, yv+j, zv+k) <= (regVal + thresVal*regVal) &&...% within range %%%%%%%%%%%Modificar
           cIM(xv+i, yv+j, zv+k) >= (regVal - thresVal*regVal) &&...% of the threshold?
