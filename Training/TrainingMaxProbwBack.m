function [tumorIntensityProb tumorTextureProb backgroundIntensityProb backgroundTextureProb] = TrainingMaxProbwBack()
...
for i=1:nImages
    ...
    tumorIntensityProb = max(tumorIntensityProb,intensityProbTumor);
    
    textureTumor = textureIm(tumor);
    textureProbTumor = imhist(textureTumor)/tumorSize(1);
    
    tumorTextureProb = max(tumorTextureProb,textureProbTumor);
    ...
    backgroundIntensityProb = max(backgroundIntensityProb,intensityProbBackground);
    
    textureBackground = textureIm(background);
    textureProbBackground = imhist(textureBackground)/backgroundSize(1);
    
    backgroundTextureProb = max(backgroundTextureProb,textureProbBackground);
